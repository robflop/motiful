...
class ListEmotesCommand extends Command {
	...
	async run(message, args) {
		...
		const emotes = readdirSync(emotePath)
			...
			.join(', ');
		if (!emotes.length) return message.edit('No custom emotes have been added.').then(msg => msg.delete(10000));
		else return message.edit(`Available custom emotes are: \`\`\`${emotes}\`\`\``).then(msg => msg.delete(10000));
	}
}
