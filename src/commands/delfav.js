...
class DelFavCommand extends Command {
	...
	async run(message, args, userData) {
		...
		if (!favoriteEmotes.hasOwnProperty(args.emoteName)) {
			return message.edit(`Emote \`${args.emoteName}\` not found on the favorites list!`).then(msg => msg.delete({ timeout: 3000 }));
		}
		delete favoriteEmotes[args.emoteName];
		message.client.logger.writeJSON(favoriteEmotes, './data/favoriteEmotes.json')
			.then(favorites => {
				message.edit(`Emote \`${args.emoteName}\` successfully removed from the favorites list!`).then(msg => msg.delete({ timeout: 2000 }));
			})
			.catch(err => {
